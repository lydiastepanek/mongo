...
def _filter_deleted_files(repo: Repo, related_test_files: Set[str]) -> Set[str]:
    ...
    """
    return {
        filepath for filepath in related_test_files if _check_file_exists_in_repo(repo, filepath)
    }


def _find_related_test_files(
    selected_tests_auth_user: str,
    ...
    """
    payload = {"threshold": 0.1, "changed_files": ",".join(changed_files)}
    ...
    related_test_files = {
        test_file["name"]
        for test_mapping in response["test_mappings"]
        for test_file in test_mapping["test_files"]
    }
    return _filter_deleted_files(repo, related_test_files)


def _create_overwrite_values(
    evg_conf: EvergreenProjectConfig, build_variant: str, task_name: str, burn_in_task_config: dict,
):
    """
...
def _generate_shrub_config(evg_api, evg_conf, expansion_file, tests_by_task, build_variant):
    ...
    for task_name, burn_in_task_config in tests_by_task.items():
        overwrite_values = _create_overwrite_values(
            evg_conf, build_variant, task_name, burn_in_task_config
        )
        ...
        suite_file_dict, shrub_config_json = GenerateSubSuites(
            evg_api, config_options
        ).generate_config_dict(shrub_config)
...
@click.option(
    "--selected-tests-auth-user",
    "selected_tests_auth_user",
    required=True,
    help="Auth user for selected-tests service.",
)
@click.option(
    "--selected-tests-auth-token",
    "selected_tests_auth_token",
    required=True,
...
def main(
    ...
    related_test_files = _find_related_test_files(
    ...
    if related_test_files:
        ...
        config_file_dict = _generate_shrub_config(
