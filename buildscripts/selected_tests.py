...
def _get_task(evg_conf: EvergreenProjectConfig, build_variant: str, task_name: str):
    """
    Look up task config for task.
    ...
    if not task:
        task = evg_build_variant.get_task(task_name + "_gen")
    return task


def _get_evg_task_configuration(task: Task, build_variant: str):
    """
...
def _generate_shrub_config(evg_api: EvergreenApi, evg_conf: EvergreenProjectConfig,
    ...
    for task_name in related_tasks:
        task = _get_task(evg_conf, build_variant, task_name)
        if not task:
            LOGGER.warning(f"The variant does not contain task", build_variant=build_variant,
                           task_name=task_name)
            continue
        else:
            evg_task_config = _get_evg_task_configuration(task, build_variant)
...
def main(
    ...
    related_tasks = _find_related_tasks(selected_tests_service, changed_files)
    LOGGER.debug("related tasks found", related_tasks=related_tasks)
    if related_tasks:
        config_dict_of_generated_tasks = _generate_shrub_config(evg_api, evg_conf, expansion_file,
                                                                related_tasks, build_variant)
